...
// Handle AJAX request to load more chapters
function load_more_chapters() {
    $offset = isset($_POST['query']['offset']) ? intval($_POST['query']['offset']) : 0;
    $chapters_per_page = isset($_POST['query']['posts_per_page']) ? intval($_POST['query']['posts_per_page']) : 5;
        ...
        while ($chapters_query->have_posts()) {
            ...
            if ($manga) {
                foreach ($manga as $manga_item) {
                    ...
                    if ($manga_post) {
                        $manga_title = $manga_post->post_title;
                        $manga_link = get_permalink($manga_item->ID);
                        ...
                        // Output the row as HTML
                        echo '<tr class="chapter-item">';
                        echo '<td class="chapter-title"><a href="' . esc_url($chapter_link) . '">' . esc_html($chapter_display_title) . '</a></td>';
                        echo '<td class="manga-name"><a href="' . esc_url($manga_link) . '">' . esc_html($manga_title) . '</a></td>';
                        echo '<td class="chapter-date">' . esc_html($chapter_date) . '</td>';
                        echo '</tr>';
                    }
                }
            }
...
function themes_styles_settings_init() {
    ...
    register_setting('themes_styles_settings_group', 'selected_homepage_style'); // New setting for homepage style
    ...
    // New field for selecting homepage style
    add_settings_field(
        'selected_homepage_style',
        'Select Homepage Style',
        'selected_homepage_style_callback',
        'themes-styles-settings',
        'themes_styles_settings_section'
    );
}
...
function themes_styles_settings_section_callback() {
    echo '<p>Select the template, theme, and homepage style for your site.</p>';
}
...
// Handle AJAX request to load more chapters for recent-chapters-2.php
function load_more_chapters_2() {
    $offset = isset($_POST['query']['offset']) ? intval($_POST['query']['offset']) : 0;
    $chapters_per_page = isset($_POST['query']['posts_per_page']) ? intval($_POST['query']['posts_per_page']) : 5;
    ...
    if ($chapters_query->have_posts()) {
        ...
        echo '<p>No more chapters found.</p>';
    }

    die(); // End AJAX request
}
add_action('wp_ajax_load_more_chapters_2', 'load_more_chapters_2');
add_action('wp_ajax_nopriv_load_more_chapters_2', 'load_more_chapters_2');
...
// New callback function for selecting homepage style
function selected_homepage_style_callback() {
    $selected_style = get_option('selected_homepage_style', 'index.php'); // Default to 'index.php'
    ?>
    <select name="selected_homepage_style">
        <option value="index.php" <?php selected($selected_style, 'index.php'); ?>>Style 1</option>
        <option value="index-2.php" <?php selected($selected_style, 'index-2.php'); ?>>Style 2</option>
    </select>
    <?php
}
...
// Function to replace index.php with index-2.php when selected as homepage style
function replace_index_template() {
    $selected_style = get_option('selected_homepage_style', 'index.php');

    if ($selected_style === 'index-2.php' && is_front_page()) {
        include(get_template_directory() . '/index-2.php');
        exit();
    }
}
add_action('template_redirect', 'replace_index_template');

function clear_cache_on_publish( $post_id ) {
    if ( wp_is_post_revision( $post_id ) ) {
        return; // Don't clear cache for revisions
    }

    // Clear the cache here
    if ( function_exists( 'w3tc_pgcache_flush' ) ) {
        w3tc_pgcache_flush(); // Clear W3 Total Cache
    }

    if ( function_exists( 'wp_cache_clear_cache' ) ) {
        wp_cache_clear_cache(); // Clear other caches
    }
}
add_action( 'publish_post', 'clear_cache_on_publish', 10, 1 );

// Function to handle AJAX request for loading latest chapters
add_action('wp_ajax_load_latest_chapters', 'load_latest_chapters');
add_action('wp_ajax_nopriv_load_latest_chapters', 'load_latest_chapters');

function load_latest_chapters() {
    // Include load-chapters.php
    include 'load-chapters.php';
