...
// Check if the current page is using the "Recent Chapters List" template
if (is_page_template('recent-chapters-list.php')) {
    // Fetch the selected template option from the database
    $selected_template = get_option('selected_recent_chapters_template', 'recent-chapters-list.php');

    // Check if the selected template is the alternative template and include it
    if ($selected_template === 'recent-chapters-list-2.php') {
        include get_template_directory() . '/recent-chapters-list-2.php';
        get_footer();
        return; // Stop further execution to prevent duplicate content
    }
}

// If not using the alternative template, continue rendering the default content below
?>
<div class="wrap">
                                    ...
                                    foreach ($manga as $manga_item) {
                                        ...
                                        if ($manga_post) {
                                            ...
                                            <tr class="chapter-item">
                                                <td class="chapter-title"><a href="<?php echo esc_url($chapter_link); ?>"><?php echo esc_html($chapter_display_title); ?></a></td>
